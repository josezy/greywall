...
func (i *bpfInstruction) writeTo(f *os.File) error {
	...
	buf := make([]byte, 8)
	buf[0] = byte(i.code) //nolint:gosec // intentional truncation for BPF encoding
	buf[1] = byte(i.code >> 8)
	buf[2] = i.jt
	buf[3] = i.jf
	buf[4] = byte(i.k)       //nolint:gosec // intentional truncation for BPF encoding
	buf[5] = byte(i.k >> 8)  //nolint:gosec // intentional truncation for BPF encoding
	buf[6] = byte(i.k >> 16) //nolint:gosec // intentional truncation for BPF encoding
	buf[7] = byte(i.k >> 24)
	_, err := f.Write(buf)
	return err
}
